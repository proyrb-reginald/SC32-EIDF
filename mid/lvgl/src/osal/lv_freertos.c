...
lv_result_t lv_thread_init(lv_thread_t *      pxThread,
    ...
    BaseType_t xTaskCreateStatus = xTaskCreate(
        prvRunThread, name, (configSTACK_DEPTH_TYPE)(usStackSize / sizeof(StackType_t)),
...
lv_result_t lv_mutex_lock_isr(lv_mutex_t * pxMutex) {
    ...
    BaseType_t xMutexTakeStatus =
        xSemaphoreTakeFromISR(pxMutex->xMutex, &xHigherPriorityTaskWoken);
    if (xMutexTakeStatus != pdTRUE) {
        LV_LOG_ERROR("xSemaphoreTake failed!");
        return LV_RESULT_INVALID;
    }
...
#    else
    ...
    while (!pxCond->xSyncSignal) {
        ...
        if (xMutexStatus == pdTRUE) {
            BaseType_t xCondWaitStatus =
                xSemaphoreTake(pxCond->xCondWaitSemaphore, portMAX_DELAY);
...
#    else
    ...
    while (ulLocalWaitingThreads > 0) {
        ...
        if (ATOMIC_COMPARE_AND_SWAP_SUCCESS ==
            Atomic_CompareAndSwap_u32(&pxCond->ulWaitingThreads, 0,
...
static void prvTestAndDecrement(lv_thread_sync_t * pxCond,
    ...
    while (ulLocalWaitingThreads > 0) {
        ...
        if (ATOMIC_COMPARE_AND_SWAP_SUCCESS ==
            Atomic_CompareAndSwap_u32(&pxCond->ulWaitingThreads,
                                      ulLocalWaitingThreads - 1, ulLocalWaitingThreads)) {
